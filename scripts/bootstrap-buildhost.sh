#!/bin/bash
# bootstrap-buildhost.sh — turn a running Linux box into a Trench build host.
#
# Creates a Gentoo stage3 chroot suitable for building Trench packages and,
...
#
# Usage:
#   sudo ./bootstrap-buildhost.sh --root /var/trench/build
#   sudo ./bootstrap-buildhost.sh --root /var/trench/build --enter
#
...
# Trench build host — generated by bootstrap-buildhost.sh
...
EOF
    ...
    # The Trench overlay is bind-mounted from the git checkout rather than
    # copied, so edits on the dev laptop land here after a plain `git pull`
    # with no extra sync step.
    cat >"$ROOT/etc/portage/repos.conf/trench.conf" <<EOF
[trench]
location = /var/db/repos/trench
sync-type =
auto-sync = no
EOF

    # XLibre is not in ::gentoo; see ARCHITECTURE.md for why Trench uses it
...
EOF
    ...
    mkdir -p "$ROOT/var/db/repos/trench"
...
Next:
  ...
  1. Bind the overlay from your git checkout:
       mount --bind /path/to/Trench/overlay $ROOT/var/db/repos/trench
